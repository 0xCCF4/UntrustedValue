...
pub fn check_data_flow<'tsrc, 'tcx>(
            ...
            DependencyGraphNode::FunctionCall { function, span: _ } => {
                if let ty::TyKind::FnDef(base, generics) = function.kind() {
                    ...
                    if base_type_str == "untrusted_value::UntrustedValue::wrap" {
                        markings.insert(node, Marking::TaintSink);
                    } else if base_type_str == "std::convert::From::from" {
                    ...
                    } else if base_type_str == "std::convert::Into::into" {
    ...
    while let Some(node) = queue.pop_front() {
        let neighbours =
            data_dependency_graph.neighbors_directed(node, petgraph::Direction::Outgoing);
    ...
    'source_test: for source in sources {
        ...
        let taint_source =
            if let Marking::TaintSource(taint_source) = markings.get(source_node).unwrap() {
                *taint_source
            } else {
                unreachable!("taint source is always found");
            };
        let source_func_sig =
            if let DependencyGraphNode::FunctionCall { function, span: _ } = source_node {
        ...
        if let Some(source) = queue.pop_front() {
            visited.push(source);
            let edge: Vec<EdgeReference<'_, GraphEdge>> = data_dependency_graph
            ...
            if edge.len() > 1 {
                taint_problems.push(TaintProblem {
                    ...
                    problem_type: TaintProblemType::Duplicated {
                        ...
                        targets: edge
                            .iter()
                            .map(|e| e.weight().instances.iter().nth(0).unwrap().span)
                            .map(|v| IRSpan::new(v, tcx))
                            .collect(),
                    },
                });
                ...
                if edge.weight().is_move_only() {
                    ...
                    taint_problems.push(TaintProblem {
                        ...
                        source_func_sig,
                        source_span: IRSpan::new(
                            edge.weight().instances.iter().nth(0).unwrap().span,
                            tcx,
                        ),
            ...
            while let Some(node) = queue.pop_front() {
                let edge = {
                    ...
                    let edge = data_dependency_graph
                        .edges_connecting(*last_node, node)
                        .nth(0)
                        .unwrap();
                ...
                if visited.contains(&node) {
                    taint_problems.push(TaintProblem {
                        ...
                        problem_type: TaintProblemType::DataDependencyLoop {
                            statement_chain: edges_traversed
                                .iter()
                                .take(edges_traversed.len() - 1)
                                .map(|e| e.instances.iter().nth(0).unwrap().span)
                                .map(|v| IRSpan::new(v, tcx))
                                .collect(),
                            loop_closure: IRSpan::new(
                                edge.instances.iter().nth(0).unwrap().span,
                                tcx,
                            ),
                        },
                    });
                ...
                visited.push(node);
                let neighbours: Vec<NodeIndex> = data_dependency_graph
                    .neighbors_directed(node, petgraph::Direction::Outgoing)
                    .collect();

                if neighbours.len() > 1 {
                    let target_edges = neighbours.iter().map(|n| {
                        data_dependency_graph
                            .edges_connecting(node, *n)
                            .nth(0)
                            .unwrap()
                            .weight()
                    });
                    ...
                    let edge = data_dependency_graph
                        .edges_connecting(node, neighbours[0])
                        .nth(0)
                        .unwrap()
                        .weight();
                                            ...
                                            taint_problems.push(TaintProblem {
                                                ...
                                                problem_type: TaintProblemType::Used {
                                                    statement_chain: edges_traversed
                                                        .iter()
                                                        .map(|e| {
                                                            e.instances.iter().nth(0).unwrap().span
                                                        })
                                                        .map(|v| IRSpan::new(v, tcx))
                                                        .collect(),
                                                    used_in: IRSpan::new(
                                    ...
                                    Marking::TaintSource(_) | Marking::Tainted => {
                                        taint_problems.push(TaintProblem {
                                            ...
                                            problem_type: TaintProblemType::Used {
                                                ...
                                                usage_type: TaintUsageType::FunctionCall(
                                                    IRSpan::new(*span, tcx),
                                                ),
                                            },
                                        });
    ...
    for index in data_dependency_graph.node_indices() {
        ...
        let (shape, name) = match node {
            DependencyGraphNode::Local { dst: local, ty: _ } => {
                ("circle", local.index().to_string())
            }
            ...
            match marking {
                Marking::TaintSource(_) => {
                    node_attributes.push(Attribute(
                        Id::Plain("color".to_owned()),
                        Id::Plain("red".to_owned()),
                    ));
                    node_attributes.push(Attribute(
                        Id::Plain("style".to_owned()),
                        Id::Plain("filled".to_owned()),
                    ));
                }
                Marking::Tainted => {
                    node_attributes.push(Attribute(
                        Id::Plain("color".to_owned()),
                        Id::Plain("orange".to_owned()),
                    ));
                    node_attributes.push(Attribute(
                        Id::Plain("style".to_owned()),
                        Id::Plain("filled".to_owned()),
        ...
        graph.push(Node::new(NodeId(Id::Plain(node_name), None), node_attributes).into());
    }
    ...
    for index in data_dependency_graph.edge_indices() {
        ...
        if let Some((src, target)) = data_dependency_graph.edge_endpoints(index) {
            ...
            let color = if matches!(
                markings.get(src_node),
                Some(Marking::Tainted | Marking::TaintSource(_))
            ) {
                "red"
            } else if matches!(
                markings.get(&data_dependency_graph[target]),
                Some(Marking::TaintSink)
            ) {
                "green"
            } else {
                "black"
    ...
    DataFlowCheckResult {
        ...
        data_dependency_graph: if draw_graph {
            Some(Graph::DiGraph {
                id: Id::Plain("Dataflowgraph".to_owned()),
                strict: false,
                stmts: graph,
            })
        } else {
